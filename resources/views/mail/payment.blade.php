        ...
        <tr>
            <td>
                <p>Thank you for choosing RadAppoint! We're delighted to inform you
                    that we have received your booking for <b>{{ $data['modality']}} - {{ $data['procedure'] }}</b> that is scheduled on
                    <b>{{ $data['date'] }}, {{ \Carbon\Carbon::parse($data['timeslot'])->format('h:i A') }}</b>. To secure your reservation, please proceed
                    with the payment process outlined below:</p>
                <ol>
